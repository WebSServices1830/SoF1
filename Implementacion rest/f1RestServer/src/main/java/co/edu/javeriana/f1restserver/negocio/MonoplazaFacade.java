...
public class MonoplazaFacade extends AbstractFacade<Monoplaza> {
    ...
    public List<Monoplaza> obtenerMonoplazasByCampeonato(int idCampeonato) {
        List<Monoplaza> res = getEntityManager().createQuery("select m "+"from Monoplaza m "+"where m.campeonato.idCampeonato = :idCampeonato order by m.marca asc", Monoplaza.class)
