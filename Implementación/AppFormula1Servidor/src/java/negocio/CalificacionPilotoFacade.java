...
public class CalificacionPilotoFacade extends AbstractFacade<CalificacionPiloto> {
    ...
    public List<Piloto> obtenerTopPilotos() {
        List<Piloto> res= getEntityManager().createQuery("select cp.piloto "+" from CalificacionPiloto cp "+"group by cp.piloto "+"order by sum(cp.puntaje)", Piloto.class).getResultList();
        if(res == null){
